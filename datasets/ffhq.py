...
def get_dataset(mode: AnyStr, args: Namespace):
    return FFHQ(mode, args, args.data_root.ffhq if args.get("data_root", False) else DATA_ROOT)


class FFHQ(ImageDataset):
    ...
    def __init__(self, mode: AnyStr, args: Namespace, data_root: AnyStr):
        ...
        if mode == utils.mode.TRAIN:
            ...
            dataset = dataset[-num_eval:]
        super(FFHQ, self).__init__(mode=mode, args=args, dataset=dataset)
        self.__verbose__()
