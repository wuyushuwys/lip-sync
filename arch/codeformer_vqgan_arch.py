    ...
    def __init__(self, in_channels, nf, emb_dim, ch_mult, num_res_blocks, resolution, attn_resolutions):
        ...
        for i in range(self.num_resolutions):
            ...
            if i != self.num_resolutions - 1:
                ...
                curr_res = curr_res // 2
        logger.info(f'Encoder with res {res}| blk_ch {in_chs}')
...
class VQAutoEncoder(nn.Module):
    def __init__(self, img_size, nf, ch_mult, quantizer="nearest", res_blocks=2, attn_resolutions=[16],
        ...
        if image_norm:
            self.register_buffer('image_mean', torch.tensor([0.5, 0.5, 0.5], requires_grad=False).view(1, 3, 1, 1))
            self.register_buffer('image_std', torch.tensor([0.5, 0.5, 0.5], requires_grad=False).view(1, 3, 1, 1))
