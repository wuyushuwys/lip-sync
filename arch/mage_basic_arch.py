...
from .fema_vqgan_arch import FeMaSRNet as VQModel
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x_query, x_kv=None, return_attn=False):
        ...
        q = q.reshape(B, N_query, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        k = k.reshape(B, N_key, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
    ...
    def forward(self, x, kv, cond=None, return_attention=False):
        if return_attention:
            ...
            if self.modulation:
                ...
                y = gated_msa.unsqueeze(1) * self.attn(self.modulate(x=self.norm1(x), shift=shift_msa, scale=scale_msa))
...
class MaskedGenerativeEncoderViT(nn.Module):
    ...
    def __init__(self, img_size=256, patch_size=16, in_chans=3,
                 ...
                 vqgan_ckpt_path='vqgan_pretrained.ckpt'):
        super().__init__()

        # --------------------------------------------------------------------------
        # VQGAN specifics
        config = OmegaConf.load('/home/user1/workspace/leilu/MAGE/config/vqgan.yaml').model
        ...
        self.vqgan_embed_dim = config.params.embed_dim
...
# model initialization (for Base/Large)
