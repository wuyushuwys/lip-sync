...
class SyncMage(DoubleConditionedMAGE):
    ...
    def __init__(self, num_embed=1024, **kwargs):
        ...
        # self.temporal_emb = PositionalEncoding(d_model=384)

        self.lip_emb = nn.Linear(self.encoder_embed_dim, 768)

        # self.lip_encoder = TransformerEncoder(embed_dim=384,
        #                                       num_heads=8,
        #                                       depth=4,
        #                                       mlp_ratio=4,
        #                                       norm_layer=partial(nn.LayerNorm, eps=1e-6))

        self.lip_proj = nn.Linear(768, num_embed)
    ...
    def forward(self, masked_img, unmasked_img=None, ref=None, audio=None, generate=False, return_loss=True):
        ...
        # with torch.no_grad():
        #     img_latent, gt_indices, token_drop_mask, token_all_mask = self.index_generator(masked_img, gt=unmasked_img)

        with torch.no_grad():
            img_latent, gt_indices, token_drop_mask, token_all_mask = self.forward_encoder(masked_img,
                                                                                           gt=unmasked_img)
        _, seq_len, emb_dim = img_latent.size()
        ...
        drop_batch = token_all_mask.sum(dim=-1).view(-1, 5).eq(0).sum(-1).bool()
        preserved_batch = ~drop_batch

        # batch dropping
        batched_img_latent = rearrange(img_latent, '(b t) n d -> b t n d', b=bsz)  # 5-frame basis
        batched_all_mask = rearrange(token_all_mask, '(b t) n -> b t n', b=bsz)[preserved_batch, ..., None]
        preserved_img_latent = batched_img_latent[preserved_batch]
        preserved_audio = audio[preserved_batch]

        # 5 frames processing
        batched_unmask = 1 - batched_all_mask
        unmask_count = batched_unmask.sum((1, 2, 3))  # unmask_count per preserved batch
        img_embed = self.lip_emb((preserved_img_latent * batched_unmask)[:, :, 1:].sum((1, 2)) / unmask_count[:, None])
        # img_latent_seq += self.temporal_emb(img_latent_seq)
        # img_embed = self.lip_encoder(img_latent_seq)
        img_embed = self.lip_proj(img_embed)
