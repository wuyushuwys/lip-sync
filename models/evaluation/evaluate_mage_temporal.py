...
def test(dataloader: DataLoader,
    ...
    for idx, (x, indiv_mels, mel, y) in enumerate(dataloader, start=1):
        ...
        (loss, acc), imgs, token_all_mask = model(x_masked,
                                                  gt=y,
                                                  ref=ref,
                                                  audio=audio_mel,
                                                  generate=True,
        ...
        save_sample_fig(x_masked, ref, imgs, y, idx, img_folder, num_batch=bsz)
...
@master_only
def save_sample_fig(x, ref, g, gt, batch_num, folder_path, num_batch):
    outputs = torch.cat([x, ref, g, gt], dim=-1)
    outputs = rearrange(outputs, '(b f) c h w -> f b c h w', b=num_batch)
    if not os.path.exists(folder_path): os.makedirs(folder_path, exist_ok=True)
    frames = []
    for output in outputs:
        output = make_grid(output, nrow=2, padding=10).permute(1, 2, 0).clamp(0, 1)
