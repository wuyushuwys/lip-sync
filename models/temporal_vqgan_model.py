...
from .evaluation import evaluate_temporal_vq

from .basic_model import BasicModel


class TemporalVQGANModel(BasicModel):
    ...
    def compile_model(self):
        self.training_g_step = torch.compile(self.training_g_step)
    ...
    def training_epoch(self, epoch):
        ...
        for batch_idx, batch in enumerate(self.train_data_loader, start=1):
            ...
            with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=self.use_amp):
                vq_info, pred_y, recon_loss, perceptual_loss, adversarial_loss, g_loss = self.training_g_step(batch)
    ...
    def evaluating_epoch(self, epoch):
        self.log_codebook()
        evaluate_temporal_vq.evaluation(model=self.g_model,
